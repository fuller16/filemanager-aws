...
<?php
    ...
    define('FM_ROOT_URL', 'http://onpoint.pinpoint.promo/order_files');
    ...
    if ($use_auth) {
        ...
        if (isset($_SESSION[FM_SESSION_ID]['logged'])) {
            ...
            if(function_exists('password_verify')) {
                $sql = "SELECT * FROM users where user = '".$_POST['fm_usr']. "' LIMIT 1";
                $result = $mysqli->query($sql);
                $row = $result -> fetch_array(MYSQLI_NUM);
                if($row && count($row) > 0 && password_verify($_POST['fm_pwd'], $row[3])){
                    ...
                    <div class="card-body">
                        <form class="form-signin" action="" method="post" autocomplete="off">
                            <div class="form-group">
                                <div class="brand">
                                    <img src="http://onpoint.pinpoint.promo/crm/dist/img/orange-logo.png">
                                </div>
        ...
        <div class="card-body">
            <p class="card-text">
                <a href="?p=<?php
                    ...
                    echo lng('Back')?></a>
                <?php
                    echo lng('DestinationFolder') ?>: <?php
                    echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH)) ?>
            </p>
            <form action="<?php
                echo htmlspecialchars(FM_SELF_URL) . '?p=' . fm_enc(rawurlencode(FM_PATH)) ?>" class="dropzone card-tabs-container" id="fileUploader" enctype="multipart/form-data">
...
<div class="path">
    ...
    <p class="break-word">
        Source path: <?php
            echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . $copy)) ?><br>
    ...
    <p>
        <b><a href="?p=<?php
            ...
            echo urlencode($copy) ?>&amp;finish=1"><i class="fa fa-check-circle"></i> Copy</a></b> &nbsp;
        <b><a href="?p=<?php
            echo urlencode(FM_PATH) ?>&amp;copy=<?php
            echo urlencode($copy) ?>&amp;finish=1&amp;move=1"><i class="fa fa-check-circle"></i> Move</a></b> &nbsp;
        <b><a href="?p=<?php
    ...
    <ul class="folders break-word">
        <?php
            if ($parent !== false) {
                ?>
        <li><a href="?p=<?php
            echo urlencode($parent) ?>&amp;copy=<?php
            echo urlencode($copy) ?>"><i class="fa fa-chevron-circle-left"></i> ..</a></li>
        <?php
            }
        ...
        <li>
            <a href="?p=<?php
                echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>&amp;copy=<?php
                echo urlencode($copy) ?>"><i class="fa fa-folder-o"></i> <?php
                echo fm_convert_win($f) ?></a>
        </li>
        <?php
            }
            ?>
    ...
    <div class="card mb-2 <?php
        ...
        <h6 class="card-header">
            <i class="fa fa-cog"></i>  <?php
                echo lng('Settings') ?>
            <a href="?p=<?php
                echo FM_PATH ?>" class="float-right"><i class="fa fa-window-close"></i> <?php
                echo lng('Cancel')?></a>
        </h6>
        <div class="card-body">
                    ...
                    <div class="col-sm-9">
                        <div class="btn-group btn-group-toggle" data-toggle="buttons">
                            ...
                            <input type="radio" name="js-error-report" id="js-err-rpt-1" autocomplete="off" value="true" <?php
                                echo getChecked($report_errors, 1, 'checked') ?> > ON
                            </label>
                            <label class="btn btn-secondary <?php
                            ...
                            <input type="radio" name="js-error-report" id="js-err-rpt-0" autocomplete="off" value="false" <?php
                                echo getChecked($report_errors, '', 'checked') ?> > OFF
                            </label>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-sm-9">
                        <div class="btn-group btn-group-toggle" data-toggle="buttons">
                            ...
                            <input type="radio" name="js-show-hidden" id="js-hdn-1" autocomplete="off" value="true" <?php
                                echo getChecked($show_hidden_files, 1, 'checked') ?> > ON
                            </label>
                            <label class="btn btn-secondary <?php
                            ...
                            <input type="radio" name="js-show-hidden" id="js-hdn-0" autocomplete="off" value="false" <?php
                                echo getChecked($show_hidden_files, '', 'checked') ?> > OFF
                            </label>
                        </div>
                    </div>
                </div>
...
<div class="row">
    <div class="col-12">
        ...
        <p class="break-word"><b><?php
            echo $view_title ?> "<?php
            echo fm_enc(fm_convert_win($file)) ?>"</b></p>
        <p class="break-word">
            Full path: <?php
                echo fm_enc(fm_convert_win($file_path)) ?><br>
            File
            size: <?php
        ...
        <p>
            <b><a href="?o=<?php echo $option; ?>&amp;p=<?php
                echo urlencode(FM_PATH) ?>&amp;dl=<?php
                echo urlencode($file) ?>"><i class="fa fa-cloud-download"></i> <?php
                echo lng('Download') ?></a></b> &nbsp;
            <b><a href="<?php
                echo fm_enc($file_url) ?>" target="_blank"><i class="fa fa-external-link-square"></i> <?php
                echo lng('Open') ?></a></b>
            &nbsp;
            <?php
                // ZIP actions
                if (!FM_READONLY && ($is_zip || $is_gzip) && $filenames !== false) {
                    ...
                    ?>
            <b><a href="?p=<?php
                echo urlencode(FM_PATH) ?>&amp;unzip=<?php
                echo urlencode($file) ?>"><i class="fa fa-check-circle"></i> <?php
                echo lng('UnZip') ?></a></b> &nbsp;
            <b><a href="?p=<?php
                echo urlencode(FM_PATH) ?>&amp;unzip=<?php
                echo urlencode($file) ?>&amp;tofolder=1" title="UnZip to <?php
                echo fm_enc($zip_name) ?>"><i class="fa fa-check-circle"></i>
            <?php
                echo lng('UnZipToFolder') ?></a></b> &nbsp;
            <?php
                }
            ...
            <b><a href="?p=<?php
                ...
                echo urlencode($file) ?>" class="edit-file"><i class="fa fa-pencil-square"></i> <?php
                echo lng('Edit') ?>
            </a></b> &nbsp;
            <b><a href="?p=<?php
                echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php
                echo urlencode($file) ?>&env=ace"
...
<div class="path">
    <div class="row">
        ...
        <div class="edit-file-actions col-xs-12 col-sm-7 col-lg-6 text-right pt-1">
            <a title="Back" class="btn btn-sm btn-outline-primary" href="?p=<?php
                echo urlencode(trim(FM_PATH)) ?>&amp;view=<?php
                echo urlencode($file) ?>"><i class="fa fa-reply-all"></i> <?php
                echo lng('Back') ?></a>
            <a title="Backup" class="btn btn-sm btn-outline-primary" href="javascript:void(0);" onclick="backup('<?php
                echo urlencode(trim(FM_PATH)) ?>','<?php
                echo urlencode($file) ?>')"><i class="fa fa-database"></i> <?php
                echo lng('BackUp') ?></a>
            <?php
                if ($is_text) { ?>
            <?php
            ...
            <a title="Advanced" class="btn btn-sm btn-outline-primary" href="?p=<?php
                echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php
                echo urlencode($file) ?>&amp;env=ace"><i class="fa fa-pencil-square-o"></i> <?php
                echo lng('AdvancedEditor') ?></a>
            <button type="button" class="btn btn-sm btn-outline-primary" name="Save" data-url="<?php
                echo fm_enc($file_url) ?>" onclick="edit_save(this,'nrl')"><i class="fa fa-floppy-o"></i> Save
            </button>
            <?php
                } else { ?>
            <a title="Plain Editor" class="btn btn-sm btn-outline-primary" href="?p=<?php
                echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php
                echo urlencode($file) ?>"><i class="fa fa-text-height"></i> <?php
                echo lng('NormalEditor') ?></a>
            <button type="button" class="btn btn-sm btn-outline-primary" name="Save" data-url="<?php
                echo fm_enc($file_url) ?>" onclick="edit_save(this,'ace')"><i class="fa fa-floppy-o"></i> <?php
                echo lng('Save') ?>
            </button>
            <?php
                } ?>
...
<form action="" method="post" class="pt-3">
    ...
    <div class="row">
        <div class="col-md-6">
            <div class="table-responsive" style="margin-bottom: 20px;">
                <table class="table table-bordered table-hover table-sm <?php
                    echo $tableTheme; ?>" id="main-table" style="width:100% !important">
                    ...
                    <tr>
                        ...
                        <td class="border-0"><a href="?p=<?php
                            echo urlencode($parent) ?>"><i class="fa fa-chevron-circle-left go-back"></i> ..</a></td>
                        <td class="border-0"></td>
                        <td style="display:none;" class="border-0"></td>
                        <td style="display:none;" class="border-0"></td>
                        <?php
                            if (!FM_IS_WIN && !$hide_Cols) { ?>
                        <td style="display:none;" class="border-0"></td>
                    ...
                    <tr>
                        ...
                        <td class="inline-actions"><?php
                            ...
                            <a title="<?php
                                ...
                                echo fm_enc(addslashes($f)) ?>');return false;"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                            <?php
                                if(isset($_GET['p'])){
                                  $get_first = explode('/', $_GET['p']);
                                  if(isset($get_first[0])){
                                      if ($get_first[0] != 'Orders'){
                                          ?>
                            <a title="<?php
                                echo lng('Delete')?>" href="?p=<?php
                                echo urlencode(FM_PATH) ?>&amp;del=<?php
                                echo urlencode($f) ?>" onclick="return confirm('<?php
                                echo lng('Delete').' '.lng('Folder').'?'; ?>\n \n ( <?php
                                echo urlencode($f) ?> )');"> <i class="fa fa-trash-o" aria-hidden="true"></i></a>
                            <?php
                                }
                                }
                                else{
                            ...
                            <a title="<?php
                                ...
                                echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="fa fa-files-o" aria-hidden="true"></i></a>
                            <?php
                                endif; ?>
                            <a title="<?php
                                echo lng('DirectLink')?>" href="<?php
                                echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f . '/') ?>" target="_blank"><i class="fa fa-link" aria-hidden="true"></i></a>
                        </td>
                    </tr>
                    <?php
                        flush();
                    ...
                    <tr>
                            ...
                            <div class="filename">
                                <?php
                                    if (in_array($ext, $doctypes)){
                                        ...
                                        $imagePreview = 'https://view.officeapps.live.com/op/embed.aspx?src='.fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . urlencode($f));
                                    }else if($ext == 'ai'){
                                        $imagePreview = 'https://docs.google.com/viewer?embedded=true&hl=en&url='.fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . urlencode($f));
                                    }else if($ext == 'eps'){
                                ...
                                <!-- <a title="<?php
                                    ...
                                    echo fm_convert_win($f) ?>" data-max-width="100%" data-width="100%"> -->
                                ...
                                <?php
                                                                       ...
                                                                       ?> 
                                <a onclick="previewlink(this)" class="previewlink" href="<?php
                                    echo fm_enc($imagePreview) ?>" title="<?php
                                    echo $f ?>" target="iframe_a">
                                <i class="<?php
                                    echo $img ?>"></i> <?php
                        ...
                        <td style="display: none;" data-sort-b="<?php
                            echo str_pad($filesize_raw, 18, "0", STR_PAD_LEFT); ?>"><span title="<?php
                        ...
                        <td style="display: none;"><?php
                            echo fm_enc($owner['name'] . ':' . $group['name']) ?></td>
                        <?php
                            endif; ?>
                        <td class="inline-actions">
                            <a style="display: none;" title="<?php
                                echo lng('Preview') ?>" href="<?php
                                echo $filelink.'&quickView=1'; ?>" data-toggle="lightbox" data-gallery="tiny-gallery" data-title="<?php
                            ...
                            <a onclick="previewlink(this)" class="previewlink" title="<?php
                                echo lng('Preview') ?>" href="https://view.officeapps.live.com/op/embed.aspx?src=<?php
                                echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . urlencode($f)) ?>" target="iframe_a"><i class="fa fa-eye"></i></a>
                            <?php
                                }else if($ext == 'ai'){ ?>
                            <a onclick="previewlink(this)" class="previewlink" title="<?php
                                echo lng('Preview') ?>" href="https://docs.google.com/viewer?embedded=true&hl=en&url=<?php
                                echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . urlencode($f)) ?>" target="iframe_a"><i class="fa fa-eye"></i></a>
                            <?php
                                }else if($ext == 'eps'){ ?>
                            <a onclick="previewlink(this)" class="previewlink" title="<?php
                                echo lng('Preview') ?>" href="/converteps.php?path=/order_files/<?php
                                echo FM_PATH.'/'.urlencode($f); ?>" target="iframe_a"><i class="fa fa-eye"></i></a>
                            <?php
                                } else if($ext == 'msg'){ ?>
                            <a onclick="previewlink(this)" class="previewlink" title="<?php
                                echo lng('Preview') ?>" href="/msgconvert.php?path=<?php
                                echo $msg_path.rawurlencode(FM_PATH).'/'.rawurlencode($f); ?>" target="iframe_a"><i class="fa fa-eye"></i></a>
                            <?php
                                } else { ?>
                            <a onclick="previewlink(this)" class="previewlink" title="<?php
                                echo lng('Preview') ?>" href="<?php
                                echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . rawurlencode($f)) ?>" target="iframe_a"><i class="fa fa-eye"></i></a>
                            <?php
                                } ?>
                            <?php
                                if (!FM_READONLY): ?>
                            ...
                            <a  title="<?php
                                ...
                                echo urlencode($f) ?> )');"> <i class="fa fa-trash-o"></i></a>
                            <a title="<?php
                                echo lng('CopyTo') ?>..."
                                href="?p=<?php
                                    echo urlencode(FM_PATH) ?>&amp;copy=<?php
                                    echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="fa fa-files-o"></i></a>
                            <?php
                                endif; ?>
                            <a title="<?php
                                echo lng('Download') ?>" href="?o=<?php echo $option; ?>&amp;p=<?php
                                echo urlencode(FM_PATH) ?>&amp;dl=<?php
                    ...
                    <tfoot style="display: none;">
                        <tr>
                            ...
                            <td colspan="<?php
                                echo (!FM_IS_WIN && !$hide_Cols) ? '6' : '4' ?>"><em><?php
                                echo 'Folder is empty' ?></em></td>
                        </tr>
                    </tfoot>
                    <?php
                        } else {
                    ...
                    <tfoot style="display: none;">
                        <tr>
                            ...
                            <td class="gray" colspan="<?php
                                ...
                                <?php
                                    echo lng('MemoryUsed').': <span class="badge badge-light">'.fm_get_filesize(@memory_get_usage(true)).'</span>' ?>
                                <?php
        ...
        <div class="col-md-6">
            <div class="iframe-responsive" >
                <p class="previewfilename">file preview</p>
                <div id="loadImg">
                    <div><img src="ajax-loader.gif"/></div>
                </div>
                <iframe src="" name="iframe_a" id="iframelink" width="100%" title="Preview Files" onload="document.getElementById('loadImg').style.display='none';">
                </iframe>
            </div>
        </div>
    </div>
    <div class="row">
        ...
        <!-- <div class="col-3 d-none d-sm-block"><a href="http://onpoint.pinpoint.promo/" target="_blank" class="float-right text-muted"><?php
            echo APP_TITLE; ?></a></div> -->
        <?php
            else: ?>
        <!-- <div class="col-12"><a href="http://onpoint.pinpoint.promo/" target="_blank" class="float-right text-muted"><?php
            echo APP_TITLE; ?></a></div> -->
        <?php
            endif; ?>
    </div>
    ...
    */
     ...
     class FM_Config
    {
        ...
        function save()
        {
            ...
            $config_string = "<?php
        " . chr(13) . chr(10) . "//Default Configuration".chr(13) . chr(10)."$var_name = $var_value;" . chr(13) . chr(10);
...
<nav class="navbar navbar-expand-lg <?php
    echo $getTheme; ?> mb-4 main-nav <?php
    echo $isStickyNavBar ?>">
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
        ...
        <div class="col-xs-6 col-sm-7 text-right">
            <ul class="navbar-nav mr-auto float-right <?php
                        ...
                        <div class="input-group-append btn-group">
                            ...
                            <div class="dropdown-menu dropdown-menu-right">
                                <a class="dropdown-item" href="<?php
                                    echo $path2 = $path ? $path : '.'; ?>" id="js-search-modal" data-toggle="modal" data-target="#searchModal">Advanced Search</a>
                            </div>
                        </div>
                    </div>
                </li>
                ...
                <li class="nav-item">
                    <a title="<?php
                        echo lng('NewItem') ?>" class="nav-link" href="#createNewItem" data-toggle="modal" data-target="#createNewItem"><i class="fa fa-plus-square"></i> <?php
                        echo lng('NewItem') ?></a>
                </li>
                <?php
                    endif; ?>
                ...
                <li class="nav-item avatar dropdown">
                    <a class="nav-link dropdown-toggle" id="navbarDropdownMenuLink-5" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> <i class="fa fa-user-circle"></i> <?php
                        if(isset($_SESSION[FM_SESSION_ID]['logged'])) { echo $_SESSION[FM_SESSION_ID]['logged']; } ?></a>
                    <div class="dropdown-menu dropdown-menu-right <?php
                        echo fm_get_theme(); ?>" aria-labelledby="navbarDropdownMenuLink-5">
                        <?php
                            if (!FM_READONLY): ?>
                        <a style="display: none;" title="<?php
...
<html lang="en">
    <head>
        ...
        <link rel="icon" href="<?php
            echo fm_enc($favicon_path) ?>" type="image/png">
        <title><?php
...
<html>
    <head>
        ...
        <link rel="stylesheet" href="<?php
            echo FM_HIGHLIGHTJS_STYLE ?>.min.css">
        <?php
            endif; ?>
        <style>
            ...
            * { -webkit-border-radius:0 !important;-moz-border-radius:0 !important;border-radius:0 !important; }
            ...
            #loadImg{position:absolute;z-index:999;top: 50%;transform: translate(-50%, -50%);left: 50%;}
    ...
    <body class="<?php
                    ...
                    <div class="modal-content <?php
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-success" onclick="newfolder('<?php
                                echo fm_enc(FM_PATH) ?>');return false;"><i class="fa fa-check-circle"></i> <?php
                        ...
                        <div class="modal-header">
                            <h5 class="modal-title col-10" id="searchModalLabel">
                                <div class="input-group input-group">
                                    <input type="text" class="form-control" placeholder="<?php
                                        echo lng('Search') ?> a files" aria-label="<?php
        ...
        <script>
            ...
            function previewlink($this){
                // console.log($($this).attr('href'));
                document.getElementById('loadImg').style.display='block';
                $('.previewfilename').html($($this).parent().parent().find('a').attr('title'));
            }
            ...
            $(document).ready( function () {
                ...
                //set iframe on page load
                //$('#iframelink').attr('src', $("a.previewlink").eq(0).attr('href'));
                //set file name for iframe on page load
                //$('.previewfilename').html($("a.previewlink").eq(0).parent().parent().find('a').attr('title'));
...
<?php
    ...
    {
        return array(
            'favicon' => ''
        );
    }
